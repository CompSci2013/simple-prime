...
/**
 * Mathematics Domain E2E Test Suite
 *
 * Tests the Mathematics discovery interface with same workflows as Automobiles
 * but configured for mathematics-specific data and fields
 */
...
test.describe('PHASE 1 (Mathematics): Initial State & Basic Navigation', () => {

  test.skip('1.1: Initial page load - mathematics records displayed', async ({ page }) => {
    // Navigate to math discover page
    await page.goto('/math/discover');
...
test.describe('PHASE 2.1 (Mathematics): Filter Workflows', () => {

  test.skip('2.1.1: Apply filter', async ({ page }: { page: Page }) => {
    await page.goto('/math/discover');

    // Apply a filter via URL (math domain has different filters)
    await page.goto('/math/discover?category=Algebra');
...
test.describe('PHASE 4 (Mathematics): Picker Panel', () => {

  test.skip('4.1.1: Single picker selection', async ({ page }: { page: Page }) => {
    // For math, picker might select category or difficulty
    await page.goto('/math/discover?pickedCategory=Algebra');

    await waitForTableUpdate(page);

    // Verify picker parameter is in URL
    const params = await getUrlParams(page);
    expect(params['pickedCategory']).toBe('Algebra');
...
test.describe('PHASE 5 (Mathematics): Statistics Panel', () => {
