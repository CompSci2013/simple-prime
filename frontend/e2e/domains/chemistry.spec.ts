...
/**
 * Chemistry Domain E2E Test Suite
 *
 * Tests the Chemistry discovery interface with same workflows as Automobiles
 * but configured for chemistry-specific data and fields
 */
...
test.describe('PHASE 1 (Chemistry): Initial State & Basic Navigation', () => {

  test.skip('1.1: Initial page load - chemistry records displayed', async ({ page }) => {
    // Navigate to chemistry discover page
    await page.goto('/chemistry/discover');
...
test.describe('PHASE 2.1 (Chemistry): Filter Workflows', () => {

  test.skip('2.1.1: Apply filter', async ({ page }: { page: Page }) => {
    await page.goto('/chemistry/discover');

    // Apply a filter via URL (chemistry domain has different filters)
    await page.goto('/chemistry/discover?type=Organic');
...
test.describe('PHASE 4 (Chemistry): Picker Panel', () => {

  test.skip('4.1.1: Single picker selection', async ({ page }: { page: Page }) => {
    // For chemistry, picker might select compound type or element
    await page.goto('/chemistry/discover?pickedType=Organic');

    await waitForTableUpdate(page);

    // Verify picker parameter is in URL
    const params = await getUrlParams(page);
    expect(params['pickedType']).toBe('Organic');
...
test.describe('PHASE 5 (Chemistry): Statistics Panel', () => {
